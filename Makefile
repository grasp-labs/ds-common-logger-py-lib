...
.PHONY: lint
lint: ## Lint with ruff
	uv run ruff check --config .config/ruff.toml --fix --exit-non-zero-on-fix $(SRC_DIR)
...
test-cov: ## Run tests with coverage (html + xml)
	uv run pytest $(TEST_DIR) --cov=$(MODULE_NAME) --cov-report=term-missing --cov-report=html --cov-report=xml
